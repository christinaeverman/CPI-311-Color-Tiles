...
float4 BlinnPixel(PhongVertexOutput input) : COLOR0
{
	...
	float diffuse = max(dot(lightDirection, input.WorldNormal), 0) * tex2D(DiffuseSampler, input.UV);
	// *** Blinn
	float specular = pow(max(dot(lightDirection + viewDirection, input.WorldNormal), 0), Shininess); // changed
	return float4(AmbientColor + diffuse * DiffuseColor + specular * SpecularColor, 1);
}
...
float4 SchlickPixel(PhongVertexOutput input) : COLOR0
{
	...
	// Now, compute the lighint components
	float diffuse = max(dot(lightDirection, input.WorldNormal), 0) * tex2D(DiffuseSampler, input.UV);
	float specular = max(dot(reflectDirection, viewDirection), 0); // changed
	return float4(AmbientColor + diffuse * DiffuseColor + specular * SpecularColor, 1);
}
...
// create two more techniques
technique Blinn
{
	pass Pass1
	{
		VertexShader = compile vs_4_0 PhongVertex(); // same vertex
